class FSW_PARAMS:
    def __init__(self):
        ...
        self.DT_CHECK_S = 0.02          # time step between important checks
        self.DT_SENSORS_S = 0.1         # time step between reading sensors
        self.DT_LOG_S  = 4              # time step between logs
        ...
        self.CHG_LIMIT_V = 3.90         # voltage to stop charging
        self.DIS_LIMIT_V = 3.20         # voltage to stop discharging
        self.FB_PULSE1_V = 3.40         # fallback voltage for 65% SOC
        self.FB_PULSE2_V = 3.30         # fallback voltage for 30% SOC
        self.SOC_PULSE1 = 0.65          # SOC to perform pulse 1
        self.SOC_PULSE2 = 0.30          # SOC to perform pulse 2
        self.TIME_CYCLE_REST_S = 12    # time to rest between charge/discharge cycles, seconds  [NOMINALLY  5 MINUTES]
        self.TIME_PULSE_REST_S = 20    # time to rest after pulse tests, seconds                [NOMINALLY 10 MINUTES]
        self.TIME_PULSE_TEST_S = 10     # time to hold current during pulse tests, seconds       [NOMINALLY 10 SECONDS]
        self.TIME_TEST_REST_S = 30    # time to rest before and after char test, seconds       [NOMINALLY  1 HOUR]
        ...
        self.CHG_LOW_SETPT_MA = -6.5    # setpoint for 1/10 C charging
        self.CHG_SETPT_DELTA_MA = 0.5   # if charging current is outside bound, increment potentiometer
        self.CHG_LOW_SETPT_DELTA_MA = 0.2 #bound for when in low charge mode
        self.DIS_SETPT_MA = 45          # discharge setpoint
        self.DIS_LOW_SETPT_MA = 6.5     # setpoint for 1/10 C discharging
        self.DIS_SETPT_DELTA_MA = 0.5
        self.DIS_LOW_SETPT_DELTA_MA = 0.2 #bound for when in low discharge mode
        self.DIS_TRANS_MA = 10
        self.CHG_VAL_INIT = 9
        self.DIS_VAL_INIT = 120
        self.R_SHUNT_OHMS = [1.0, 1.0, 1.0]
        self.NUM_CYCLES_PER_TEST = 2    # typically 20
