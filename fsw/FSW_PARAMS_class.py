...
class FSW_PARAMS:
    def __init__(self, file_name):
        ...
        try:
            ...
            self.DT_LOG2_S = 180             # time step between type 2 logs
            self.DT_LOG3_S = 900            # time step between type 3 logs
            self.DT_HEAT_S = 90            # time step between checking heater
            ...
            self.NUM_CYCLES_PER_TEST = 100    # typically 100
            self.num_boots = 0
            self.ALPHA_EKF = 0.04
            self.ALPHA_CYC = 0.02
            ...
            self.Q_state = np.diag([1e-6, 1])     # Process noise covariance
            self.Q_param = 1e-1                      # Parameter process noise
            self.R_param = 4.1e-4                # Parameter measurement noise
